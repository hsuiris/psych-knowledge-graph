...
諮商與臨床心理學的核心概念，用一張可以點的圖譜串起來。109 個概念、33 位代表人物、14 個經典研究、313 條關係，分成八類。點一個節點，右側會出現分段的深入說明、人物照片與實驗圖片，說明裡提到的其他概念可以直接點過去，再沿著關係往下探索。
...
涵蓋諮商理論課程的主要學派：精神分析、阿德勒學派、存在主義、個人中心、完形、行為、認知行為、現實、女性主義、後現代取向與家庭系統治療，另有遊戲治療、沙遊、藝術治療等取向，以及積極傾聽、面質、立即性等基本諮商技巧。心理疾患依 DSM-5 分得更細，例如焦慮症底下有恐慌症、社交焦慮症、廣泛性焦慮症與特定畏懼症。
...
156 個節點各有一頁，含定義、分段說明、附圖、關係列表與參考書目，網址可以直接分享。內文一樣會自動連到其他概念頁。這些頁面是靜態產生的，搜尋引擎爬得到。
...
- 樞紐概念（連結數 6 以上）在任何縮放層級都顯示名稱，其他節點放大後才顯示。

## 技術

| 項目 | 選擇 | 理由 |
| --- | --- | --- |
| 框架 | Next.js 16 App Router + TypeScript | 156 個概念頁用 `generateStaticParams` 靜態產生 |
| 圖譜 | react-force-graph-2d（Canvas） | 156 節點的力導向布局，拖曳與縮放都順 |
| 搜尋 | Fuse.js | 純前端模糊比對，不需要後端或金鑰 |
| 樣式 | Tailwind CSS 4 | |
| 部署 | GitHub Pages，靜態輸出 | 全站零後端、零金鑰，167 頁全部預先產生 |
